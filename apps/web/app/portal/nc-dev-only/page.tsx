        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 16, fontSize: 13, color: "var(--color-on-surface-variant)" }}>
              ...
              <span style={{ display: "flex", alignItems: "center", gap: 6 }}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>payments</span>${revenue.toFixed(2)} Revenue</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 20, marginBottom: 40 }}>
            {[
              ...
              { label: "TOTAL REVENUE", value: `$${revenue.toFixed(2)}`, icon: "payments" },
                ...
                {active.map((order) => {
                  ...
                  return (
                    <div key={order.orderId} style={{ minWidth: 290, background: "var(--color-surface-container-lowest)", borderRadius: 16, border: "1px solid rgba(210,196,186,0.2)", padding: 20, flexShrink: 0, transition: "all 0.2s", opacity: busy ? 0.6 : 1 }}>
                      ...
                      <p style={{ fontSize: 13, fontWeight: 700, color: "var(--color-secondary)", marginBottom: 12 }}>${order.total.toFixed(2)}</p>
                      {cfg && (
                        <button onClick={() => handleAction(order)} disabled={busy} style={{ width: "100%", padding: "10px 16px", background: busy ? "rgba(210,196,186,0.4)" : cfg.actionBg, color: busy ? "var(--color-on-surface-variant)" : cfg.actionColor, fontFamily: "'Be Vietnam Pro', sans-serif", fontSize: 13, fontWeight: 700, border: "none", borderRadius: 10, cursor: busy ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "var(--color-surface-container-low)", borderBottom: "1px solid rgba(210,196,186,0.2)" }}>
                    {["Customer", "Table", "Order ID", "Status", "Total", "Waktu"].map((h) => (
                      <th key={h} style={{ padding: "14px 20px", textAlign: h === "Waktu" ? "right" : "left", fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--color-on-surface-variant)", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.length === 0 && (
                    <tr><td colSpan={6} style={{ padding: "40px 24px", textAlign: "center", color: "var(--color-on-surface-variant)", fontSize: 14, opacity: 0.6 }}>
                  ...
                  {filtered.map((order, i) => {
                    ...
                    return (
                      <tr key={order.orderId} style={{ borderBottom: "1px solid rgba(210,196,186,0.08)", transition: "background 0.15s" }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "rgba(245,236,231,0.4)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}>
                        ...
                        <td style={{ padding: "14px 20px", fontSize: 14, fontWeight: 700, color: "var(--color-primary)" }}>${order.total.toFixed(2)}</td>
